...
#------------------------------------#
# 문제 풀이 핵심 아이디어
# 1. 스택 두 개를 만드ㅏㄹ고, 스택 두 개의 중간 지점을 커서(Cursor)로 간주한다.
...
for _ in range(test_case):
    ...
    for i in data:
        if i == '-':
            if left_stack:              # 비어있지 않을때만 지우면 되므로
                left_stack.pop()
        elif i == '<':
            if left_stack:
    ...
    left_stack.extend(reversed(right_stack))        # append하면 뒤부터 쌓이므로 오른쪽 스택을 거꾸로 바꿔준 뒤 합쳐준다.
