...
# 문제 해결
## 2차원 화면에 포현하므로 최소신장트리
## 정점의 개수 N이 최대 1,000이므로, 가능한 통로의 개수는 약 N^2
## 크루스칼 알고리즘은 간선의 개수가 E일 때 O(ElogE)로 동작한다.
## 따라서 크루스칼 알고리즘으로 해결이 가능하다.
...
def get_parent(parent, n):                          # 크루스칼 알고리즘은 사이클이 없으므로 union-find 알고리즘까지 사용해준다.
    if parent[n] == n:
        return n
    return get_parent(parent, parent[n])

def union_parent(parent, a, b):                     # 크루스칼 알고리즘은 사이클이 없으므로 union-find 알고리즘까지 사용해준다.
...
def find_parent(parent, a, b):                      # 크루스칼 알고리즘은 사이클이 없으므로 union-find 알고리즘까지 사용해준다.
    a = get_parent(parent, a)
    b = get_parent(parent, b)
    if a == b:
        return True
