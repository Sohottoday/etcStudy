...
# 하지만 욕심 많고 제멋대로인 유치원 아이들은 차례대로 받으라는 선생님의 말을 무시한 채 마구잡이로 사탕을 집어 갔고 
# 많은 사탕을 집어 간 아이가 있는가 하면 사탕을 거의 차지하지 못하고 우는 아이도 있었다. 
...
# 자기가 가진 사탕의 반을 옆에 오른쪽에 앉은 아이에게 주는 과정과 선생님이 사탕을 보충해 주는 과정을 묶어서 1 순환이라고 할 때 
# 몇 번의 순환을 거치면 모든 아이들이 같은 수의 사탕을 가지게 되는지 계산 해보자. 
...
def check(N, candy):
    for i in range(N):
        if candy[i] % 2 == 1:
            candy[i] += 1
    return len(set(candy)) == 1     # set는 같은 수가 존재할 경우 하나로 합친다는 것을 활용하여 set의 길이가 1이면 리턴한다는 의미

def teacher(N, candy):
    tmp_lst = [0 for i in range(N)]
    for idx in range(N):
        if candy[idx] % 2:
            candy[idx] += 1         # 홀수면 1을 더해준다.
        candy[idx] //= 2
        tmp_lst[(idx+1) % N] = candy[idx]       # 오른쪽으로 전달
        
    for idx in range(N):
        candy[idx] += tmp_lst[idx]      # 기존에 가지고 있던 것에 더해준다.
