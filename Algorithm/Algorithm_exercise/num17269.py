...
lst = [alp[ord(i)-ord('A')] for i in AB]        # 문자를 아스키 값으로 변환하여 아스키 A값을 빼주면 인덱스를 알 수 있다. 이것을 반복한다는 의미
## 위의 인덱스를 구하는 논리적인 부분을 생각할 수 있어야 한다.

for i in range(N+M-2):      # 2개 남을때 까지이므로 -2
    for j in range(N+M-1-i):        # 어디서부터 어디까지 해야 하느냐 -> ex) 첫번째 수행은 끝까지 진행되므로
        lst[j] += lst[j+1]
