...
class Node:
    def __init__(self, number, left_node, right_node):      
        self.parent = -1                        # 노드 1번이 항상 루트노드가 되는건 아니므로 부모값을 기록하도록 한다.
        self.number = number
...
for i in range(1, n+1):
    tree[i] = Node(i, -1, -1)           # 왼쪽과 오른쪽이 모두 비어있다고 가정하고 노드를 초기화시킴
    level_min.append(n)
    level_max.append(0)
...
for _ in range(n):
    ...
    if left_node != -1:                 # 왼쪽 노드가 존재한다면~ 이라는 의미
        tree[left_node].parent = number 
    if right_node != -1:                # 오른쪽 노드가 존재한다면~ 이라는 의미
