...
## 사실상 정렬만 수행하면 되므루 O(NlogN)으로 문제를 해결할 수 있다.

## 따라서
## 각 센서를 오름차순 정렬
## 각 센서 사이의 거리를 계산
## 가장 거리가 먼 순서대로 k - 1개의 연결고리를 제거한다.

import sys
n = int(input('센서의 개수를 입력하시오 : '))
k = int(input('기지국의 수를 입력하시오 : '))
...
array = list(map(int, input('센서위 위치를 입력하시오 : ').split(' ')))
array.sort()
